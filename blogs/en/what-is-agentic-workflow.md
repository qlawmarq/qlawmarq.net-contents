...
description: "Exploring AI agent workflows: differences from LLMs, building tools like LangGraph and n8n, token limit handling, and hallucination countermeasures"
...
Since you're using AI agents, if there are tools or APIs that AI agents can use, you can incorporate them into the workflow. You can have AI agents perform web searches or retrieve information from external databases.
Many companies have already deployed customer support chatbots, and by using **RAG (Retrieval-Augmented Generation)**, it's possible to generate information while referencing external knowledge bases and documents.
...
Also, I've recently been paying attention to Google's ADK, which came later. While LangGraph requires using LangSmith separately for monitoring and debugging, ADK has various features including monitoring, debugging, and evaluation built in, which is attractive (though I haven't used ADK in actual production yet, so it's still an unknown).
...
LangGraph proposes using the [Map-Reduce pattern](https://langchain-ai.github.io/langgraph/how-tos/map-reduce/) to split large datasets, process each part individually, and then integrate the results, so utilizing such patterns is also a good approach.
...
### Dealing with Ambiguity in User Intent

This has long been a challenge in fields like search engines and recommendation engines, but when user input is ambiguous or context-dependent, it can be difficult for AI agents to accurately understand the intent.

Especially when using RAG (Retrieval-Augmented Generation) to reference external knowledge bases and documents, it becomes important to accurately convey user intent to the AI agent.
For example, an instruction like "Analyze recent sales data and create a report" is unclear about details such as "what period" or "which metrics to prioritize," and the report generated by the AI agent may not meet user expectations.

There are several ways to address this problem.

One is simply to provide guidelines or templates so users can give more specific instructions. For example, preparing templates like "When creating a sales data analysis report, please include the following items: period, key metrics, trend analysis, recommended actions." However, this is a user-dependent solution, so it's not universal.

Another is to have multiple AI agents repeat dialogue until user intent and requirements become clear. For example, the first AI agent receives user instructions, asks additional questions until user requirements are clear, and once all user requirements are clear, the next AI agent performs information retrieval and report creation.

```mermaid
flowchart TD
    A[User Instruction] --> B[AI Agent 1: Confirm Intent]
    B --> C{Is requirement clear?}
    C -- No --> D[Ask additional questions]
    D --> B
    C -- Yes --> E[AI Agent 2: Create Report]
    E --> F[Generate Report]
```

Due to the nature of LLMs, it's difficult to respond 100% accurately to user requirements, but by implementing such measures, it's possible to more accurately grasp user intent and increase the likelihood of generating output that meets expectations.
